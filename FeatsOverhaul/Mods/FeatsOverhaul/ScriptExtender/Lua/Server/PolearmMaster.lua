...
local function isVersatile(weapon)
    return (H.GetEntity(weapon).Weapon.WeaponProperties & 2048 > 0)
end
...
Ext.Osiris.RegisterListener("Equipped", 2, "after", function(item, character)
    ...
    if Osi.GetEquipmentSlotForItem(item) == 4 then
        ...
        if Osi.HasActiveStatus(mainHandWeapon, "CHT_REACH_OVERWRITE") == 1 then
...
Ext.Osiris.RegisterListener("Unequipped", 2, "after", function(item, character)
    ...
    if Osi.HasActiveStatus(item, "CHT_REACH_OVERWRITE") == 1 then
        Osi.RemoveStatus(item, "CHT_REACH_OVERWRITE")
    end
...
Ext.Osiris.RegisterListener("StatusApplied", 4, "after",
    function(weapon, status, causee, storyId)
        ...
        local entity = H.GetEntity(weapon)
        ...
        -- -- Debugging
        -- local displayName = H.GetDisplayName(weapon)
...
Ext.Osiris.RegisterListener("StatusRemoved", 4, "after",
    function(weapon, status, causee, storyId)
        ...
        -- read StatsId after the nil-check, not before, or a missing entity errors here
